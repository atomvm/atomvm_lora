...
set_sync_word(SPI, SyncWord) ->
    ?LOG_DEBUG("set_sync_word(~p)", [SyncWord]),
...
set_current_limit(SPI, CurrentLimit) ->
    ?LOG_DEBUG("set_current_limit(~p)", [CurrentLimit]),
...
set_sleep(SPI, Start, RTC) ->
    ?LOG_DEBUG("SetSleep(~p, ~p)", [Start, RTC]),
...
set_standby(SPI, StandbyConfig) ->
    ?LOG_DEBUG("SetStandby(~p)", [StandbyConfig]),
...
%     ?LOG_DEBUG("StopTimerOnPreamble(~p)", [Value]),
%     Data = <<Value:8>>,
...
%     ?LOG_DEBUG("SetRxDutyCycle(~p, ~p)", [RxPeriod, SleepPeriod]),
...
%     ?LOG_DEBUG("SetTxInfinitePreamble()", []),
...
calibration_function(SPI, CalibParam) ->
    ?LOG_DEBUG("CalibrateFunction(~p)", [CalibParam]),
    Data = <<CalibParam:8>>,
...
calibrate_image(SPI, FreqBand) ->
    ?LOG_DEBUG("CalibrateImage(~p)", [FreqBand]),
...
set_pa_config(SPI, PaDutyCycle, HpMax, DevSel, PaLut) ->
    ?LOG_DEBUG("SetPaConfig(~p, ~p, ~p, ~p)", [PaDutyCycle, HpMax, DevSel, PaLut]),
...
set_rx_tx_fallback_mode(SPI, FallbackMode) ->
    ?LOG_DEBUG("SetRxTxFallbackMode(~p)", [FallbackMode]),
    Data = <<FallbackMode:8>>,
...
write_register(SPI, Address, Data) ->
    ?LOG_DEBUG("WriteRegister(~p, ~p)", [Address, Data]),
...
read_register(SPI, Address, Len) ->
    ?LOG_DEBUG("ReadRegister(~p, ~p, ~p)", [SPI, Address, Len]),
...
write_buffer(SPI, Offset, Data) ->
    ?LOG_DEBUG("WriteBuffer(~p, ~p)", [Offset, Data]),
...
read_buffer(SPI, Offset, Len) ->
    ?LOG_DEBUG("ReadBuffer(~p, ~p)", [Offset, Len]),
    NopPayload = create_nop_payload(Len + 1, []),
...
set_dio_irq_params(SPI, IRQMask, DIO1Mask, DIO2Mask, DIO3Mask) ->
    ?LOG_DEBUG("SetDioIrqParams(~p, ~p, ~p, ~p)", [IRQMask, DIO1Mask, DIO2Mask, DIO3Mask]),
...
clear_irq_status(SPI, Mask) ->
    ?LOG_DEBUG("ClearIrqStatus(~p)", [Mask]),
    Data = <<Mask:16>>,
...
set_dio2_as_rf_switch_ctl(SPI, Enable) ->
    ?LOG_DEBUG("SetDIO2AsRfSwitchCtrl(~p)", [Enable]),
    Data = <<Enable:8>>,
...
set_dio3_as_tcxoc_ctl(SPI, Voltage, Delay) ->
    ?LOG_DEBUG("SetDIO3AsTCXOCtrl(~p, ~p)", [Voltage, Delay]),
...
set_packet_type(SPI, PacketType) ->
    ?LOG_DEBUG("SetPacketType(~p)", [PacketType]),
    Data = <<PacketType:8>>,
...
set_tx_params(SPI, Power, RampTime) when -9 =< Power andalso Power =< 22 andalso 16#00 =< RampTime andalso RampTime =< 16#07  ->
    ?LOG_DEBUG("SetTxParams(~p, ~p)", [Power, RampTime]),
...
set_modulation_params(SPI, SpreadingFactor, BandWidth, CodingRate, LowDataRateOptimize) ->
    ...
    ?LOG_DEBUG("SetModulationParams(~p, ~p, ~p, ~p)", [SF, BW, CR, LDRO]),
...
set_packet_params(SPI, PreambleLength, HeaderType, PayloadLength, CRCType, InvertIQ) ->
    ...
    ?LOG_DEBUG("SetPacketParams(~p, ~p, ~p, ~p, ~p)", [PreambleLength, HT, PayloadLength, CRC, IIRQ]),
...
set_cad_params(SPI, CadSymbolNum, CadDetPeak, CadDetMin, CadExitMode, CadTimeout) ->
    ?LOG_DEBUG("SetCadParams(~p, ~p, ~p, ~p, ~p)", [CadSymbolNum, CadDetPeak, CadDetMin, CadExitMode, CadTimeout]),
...
set_buffer_base_address(SPI, TXBaseAddress, RXBaseAddres) ->
    ?LOG_DEBUG("SetBufferBaseAddress(~p, ~p)", [TXBaseAddress, RXBaseAddres]),
...
write_command({SPI, DeviceName}, OpCode, Data) ->
    ...
    % ?LOG_DEBUG("[erl] write [~s]", [atomvm_lib:to_hex(Payload)]),
...
write_read_command({SPI, DeviceName}, OpCode, Data) ->
    ...
    % ?LOG_DEBUG("[erl] write-read [~s] -> [~s]", [atomvm_lib:to_hex(Payload), atomvm_lib:to_hex(Response)]),
    Response.
